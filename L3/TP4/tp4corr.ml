...
(* Entrees :
 * - x et y : les coordonn�es du point (entiers)
 * - longueur : longueur de l'image (int)
 * - arbre : repr�sentation de l'image (image_tree)
 *
 ...
 * - arbre : repr�sentation par un arbre de l'image (image_tree)
 *
 * Sortie : repr�sentation par un tableau de l'image (color array array)
 *
 * Algo :
 * Cr�er un tableau et pour chaque �l�ment x (i.e une ligne du tableau
 * qui repr�sente une colonne de l'image)
 *   Cr�er un tableau et pour chaque �l�ment y (i.e le pixel de la ligne y
 * de la colonne x)
...
(* Entrees :
 * - matrix : repr�sentation matricielle de l'image (color array array)
 * - x et y : coordonn�es (int)
 * - length : taille du carr� � consid�rer (int)
 * - color : couleur � chercher (Graphics.color)
 *
 * Sortie :
 * est-ce le carre de (x,y) a (x+length-1,y+length-1) ne contient que color ? (bool�en)
 *
 * Algo :
...
(* Entree :
 * - matrix : repr�sentation matricielle de l'image
 * (color array array)
 * - x et y : coordonn�es (int)
 * - length : taille du carr� � consid�rer (int)
 *
 * Sortie :
 * repr�sentation en arbre du carr� (x,y) -> (x+length-1, y+length-1) (image_tree)
 *
 * Algo :
 * Si le carr� est uniform�ment noir
 *   renvoie F black
 * Si le carr� est uniform�ment blanc
 *   renvoie F white
 * Sinon
 *   calcule r�cursivement les repr�sentations des 4
...
(* Entree : matrix : repr�sentation matricielle de l'arbre (color array array)
 *
 * Sortie : repr�sentation en arbre de l'image (image_tree)
 *
 * Algo :
